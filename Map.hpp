...
typename Map<K, V, C>::Iterator Map<K, V, C>::find(const K& k) const {
  // Create a dummy pair with the key to search for
  // The value doesn't matter since PairComp only compares keys
  Pair_type dummy = {k, V()};
...
V& Map<K, V, C>::operator[](const K& k) {
  ...
  // If found, return reference to the value
  if (it != end()) {
    return it->second;
  }

  // If not found, insert new element with default-initialized value
...
Map<K, V, C>::insert(const Pair_type &val) {
  ...
  if (it != end()) {
    // Key exists, return iterator to existing element and false
    return {it, false};
  }
